...
ReactDOM.render(
  <React.Fragment>
    ...
    <p>
      ...
      called <b>b-threads</b> (aka function generators)
      that run in parallel and can{' '}
...
function* detectWinByX() {
  ...
  yield {
    ...
    <p>
      A b-thread can yield an object with a key being any
      of these 3 operators. When the key is{' '}
      <span style={{ color: 'green' }}>wait</span>, the
      execution of the generator will stop at that section
      until such event is triggered by other b-threads or
    ...
    <p>
      ...
      only a proposal for that event to be considered for
      triggering.
    </p>
    <p>
    ...
    <p>
      ...
      effectively "blocking until another event happens".
      Or one can yield a request with a wait, effectively
    ...
    <p>
      ...
      row and then requests XWins. Try drawing 3 X's on the
      board below:
    </p>
    ...
    <p>
      ...
      events, they need to form a line. More specifically
      in fact we need to create 8 threads, each of which
      listen out for a specific line and then declare a
      win:
    </p>
    <p>
      <pre>
...
...generateThreads(
  ...
  ([cell1, cell2, cell3]) =>
    ...
    <p>
      ...
      we should not be able to trigger two X's or two O's
      in a row. In other words, when X goes, only O is
      allowed after it and so on. This is how the game
      works!
    </p>
    <p>
      <pre>
...
function* enforcePlayerTurns() {
  while (true) {
    ...
    <p>
      ...
      without having to dig and figure out how
      earlier-written code works!
    </p>
    <p>
  ...
  yield {
    ...
    <p>
      <blockquote>
        ...
        ability allows programmers to “sculpt” the system
        under construction, adding, removing or suppressing
        behaviors, with little or no need to modify the
        code of existing b-threads.
      </blockquote>
    </p>
...
const defaultMoves = generateThreads(
  ...
  cellNumber =>
    ...
    <p>
      ...
      Normally we'd have to dig into already-written code
      to figure something like this out, possibly changing
  ...
  yield {
    ...
    <p>
      <blockquote>
        ...
        Initial or partial versions of behavioral programs
        can be executed and can often display meaningful
        system behavior. This allows users and developers
        to observe the system’s behavior at any stage of
    ...
    <p>
      ...
      simply putting O's on the board in order from
      top-left to bottom-right.
    </p>
    <p>
      We want to have a b-thread observe when the user has
      drawn two X's in a line and then forcibly request an
      O (again with higher priority) so that it stops X
      from winning.
    </p>
    <p>
      <pre>
...
>
  <Wrapper>
    ...
    <p>
      ...
      way of programming. If you want to read more about it
      I highly suggest you read academics paper about it on
